...
class ResultsViewer:
    ...
    def print_header(self):
        ...
        print()
        print("=" * 120)
        print(f"{'Date/Time':<20} {'Task':<25} {'Method':<30} {'Time':<15} {'Ops/sec':<12} {'Threads':<8}")
        print("=" * 120)
    ...
    def print_results(self, results: List[Dict]):
        ...
        for r in results:
            ...
            task_name = r.get('task_name', f"Task {r.get('task_number', '?')}")[:24]
            method_name = r.get('method_name', 'Unknown')[:29]
            ...
            print(f"{timestamp:<20} {task_name:<25} {method_name:<30} {time_str:<15} {ops_str:<12} {threads:<8}")
        
        print("-" * 120)
        print(f"Total records: {len(results)}")

    def print_statistics(self, task_number: Optional[int] = None):
        ...
        for task in tasks:
            stats = self.db.get_task_statistics(task)
            if not stats:
                continue
                
            print(f"\n📌 Task {task}")
            print("-" * 80)
            print(f"{'Method':<35} {'Runs':<10} {'Average':<15} {'Min':<15} {'Max':<15}")
            print("-" * 80)
            
            for s in stats:
                method = s.get('method_name', 'Unknown')[:34]
                count = s.get('count', 0)
                ...
                print(f"{method:<35} {count:<10} {avg:<15} {min_t:<15} {max_t:<15}")
    ...
    def export_csv(self, results: List[Dict], filename: str = "benchmark_results.csv"):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=[
                'timestamp', 'task_number', 'task_name', 'method_name',
                'execution_time_ns', 'operations_per_second', 'thread_count', 'parameters'
            ])
            writer.writeheader()
  ...
  python3 view_results.py --watch            # Monitoring
    ...
    try:
        if args.watch:
            ...
            viewer.print_statistics(task_number=args.task)
        elif args.export:
